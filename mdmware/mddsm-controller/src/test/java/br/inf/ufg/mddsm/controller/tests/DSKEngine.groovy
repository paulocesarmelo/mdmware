...
class DSKEngine {
	...
	static void loadMetaModels() {
		...
		for(content in dsml.eContents()){
			...
			//println(content.getEAllSuperTypes().name[0] )
			/*if(content.eContents().getAt(1) instanceof EGenericTypeImpl) {
				
				if(content.eContents().getAt(1).getEClassifier().name.equals("EActor") || content.eContents().getAt(1).getEClassifier().name.equals("EItem")) {
					println("name: "+content.name)
					entityNames.add(content.name)
				}
			}*/

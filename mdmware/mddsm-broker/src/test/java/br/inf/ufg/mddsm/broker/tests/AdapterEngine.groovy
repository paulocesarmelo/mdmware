...
class AdapterEngine {
	...
	static final HOME_PATH = "/home/paulo/Documentos/Repository/principal/mddsm/mddsm-broker"
	static final CONTROLLER_PATH = "/home/paulo/Documentos/Repository/principal/mddsm/mddsm-controller"
	...
	public static void main(String[] args) {
		...
		domainName = JOptionPane.showInputDialog("Informe o nome domínio:");
	...
	/*static void test() {
		...
		names.add("paulo")
		names.add("deu certo")
		shell.setVariable("lista", names);
		shell.setVariable("n", "test");
		String cmd = "for (x=0; x<2; x++){println(lista.get(x)) }; return(x);";
	...
	static void loadElements() {
		...
		String BROKER_MODEL_PATH = "$HOME_PATH-model/src/main/resources/model"
		String CONTROLLER_MODEL_PATH = "$CONTROLLER_PATH-model/src/main/resources/model"
		
		
		def psk_model = "$BROKER_MODEL_PATH/psk2"+platformName+".xmi"
		def dsk_model = "$CONTROLLER_MODEL_PATH/dsk2"+domainName+".xmi"
		
		def adapt_model = "$BROKER_MODEL_PATH/adpt2"+domainName+"2"+platformName+".xmi"
		
		adapter = modelHandler.load(adapt_model.toURI())
	...
	/*static void loadDSCs () {
		...
		def model = "$MODELS_PATH/adpt-"+domainName+"-"+platformName+".xmi"

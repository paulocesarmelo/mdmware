...
import br.inf.ufg.mddsm.broker.im.metadata.DSCCall;
import br.inf.ufg.mddsm.broker.im.metadata.EUCall;
...
public class IntentModelExecutor {
	
	IntentModel model;
	Procedure currentProcedure;
	ExecutionStateManager stateManager = ExecutionStateManager.getInstance();
	private static final Logger log = LoggerFactory.getLogger(IntentModelExecutor.class);
	
	UUID id;
	
	public IntentModelExecutor(){
		id = UUID.randomUUID();
	}
	...
	public void executeModel(IntentModel intentModel) throws CompileException, InvocationTargetException{
		log.trace("executeModel(im:{})", intentModel);
		stateManager.putAttribute(new Attribute(intentModel.getId(), intentModel));
		System.out.println("DSC Initial Name: "+intentModel.getInit().getClassifier().getName());
		System.out.println("StartEU id: "+ intentModel.getInit().getStartEU().getId());
	...
	public void executeModel(EventCallBack modelCallback) throws CompileException, InvocationTargetException{
		System.out.println("executeModel2");
		...
		System.out.println("EU: "+startEU.getBody());
		startEU.setBody(startEU.getBody());
